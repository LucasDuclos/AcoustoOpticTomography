...
def soft_clip_edges(y, edge_fraction=0.1):
    """
    y: (Nt, N_angles)
    edge_fraction: 比例，控制两端被衰减的长度
    """
    Nt = y.shape[0]
    edge_len = int(Nt * edge_fraction)

    # left , right windows
    left_win = windows.hann(2 * edge_len)[:edge_len]
    right_win = windows.hann(2 * edge_len)[edge_len:]

    # left , 1 , right window
    window = np.ones(Nt)
    window[:edge_len] = left_win
    window[-edge_len:] = right_win

    return y * window[:, np.newaxis]
...
def getSaveAOsignal(listFieldPathHDR, laser, acoustic_envelope_squaredMATRIX, save_directory, WINDOW):
    
    AOsignal = getAOsignal(laser, acoustic_envelope_squaredMATRIX)

    if WINDOW == 1 :
        AOsignal = soft_clip_edges(AOsignal, edge_fraction=0.1) 
    ...
    for i in range(len(active_lists)):
        # print(f"Parsing active list {i}: {active_lists[i]}")
