classdef OS < TF2D
    ...
    methods
        ...
        function obj = OS(InputImage,theta,decimation,df0x,z,SamplingRate,c,Lx)
            %N: number of points for fourier transform
%            N = 2^10 ;
%            Fmax_x = N*df0x;
%            Fmax_z = 1/(c*(t(2)-t(1))) ;
            %obj@TF2D(N,N,Fmax_x,Fmax_z);
            obj@TF2D( linspace(Lx(1),Lx(2),2^10),z);
            
            if (size(InputImage,1) == length(z) && size(InputImage,2) == length(theta))
            % checkin that dimension match the input image
            obj.SamplingRate = SamplingRate ;
            obj.ct = z; % longitudinal index for reconstruction box
            obj.c = c ;
            obj.Lx = Lx ;
            obj.Lz = [min(z),max(z)] ;
            obj.R = InputImage;
            ...
            %obj.R = interp1(c*t,obj.R,obj.z,'linear',0);
            obj.R = InputImage ;
        ...
        function Iout = InverseFourierX(obj,Iin,decimation,theta, C )
            ...
            decimation = unique(decimation);
            [Angles,ia,ib] = unique(theta) ;
            % ia : index of angles in Angles reduced vector
            % ib : index of angle in intial theta vector with all
            % frequency structuration
            Ntheta = length(Angles);
            I0 = obj.Nx/2 + 1 ;
...
% function Ireconstruct = iRadon(obj, X_m , DelayLAWS , ActiveLIST , df0x , Lobject)
% 
%     % === Prétraitement des lois de délai ===
%     DelayLAWS_  = obj.SqueezeRepeat(DelayLAWS);
%     ActiveLIST_ = obj.SqueezeRepeat(ActiveLIST);
% 
%     [theta,~,~,M0] = EvalDelayLawOS_shared(X_m,DelayLAWS_,ActiveLIST_,obj.c);
% 
%     % === Compression sin/cos (structuration) ===
%     [F_ct_kx , theta , decimation] = obj.AddSinCos(obj.R);
% 
%     % === TF selon z ===
%     Fin = obj.fourierz(F_ct_kx);      % [fz , k]
% 
%     % === Filtre de Ram-Lak / objet ===
%     FILTER = obj.GetFILTER(Lobject,size(Fin,2));  % Nz × Nk
% 
%     fz = obj.fz(:);     %  vecteur colonne Nz × 1
%     Nz = length(fz);
%     Nk = length(decimation);
% 
%     % === Allocation ===
%     Finf = zeros(Nz,Nk,'like',Fin);
%     Fsup = zeros(Nz,Nk,'like',Fin);
% 
%     % === Filtrage dépendant de la décimation ===
%     for i = 1:Nk
% 
%         fx_i = abs(decimation(i)) * df0x;   % fréquence spatiale associée
% 
%         % Support spectral OS
%         Hinf_i = abs(fz) < fx_i;            % Nz × 1
%         Hsup_i = fz >= 0;                   % Nz × 1
% 
%         Finf(:,i) = Fin(:,i) .* FILTER(:,i) .* Hinf_i;
%         Fsup(:,i) = Fin(:,i) .* FILTER(:,i) .* Hsup_i;
%     end
% 
%     % === Retour espace z ===
...
% 
%     % === Grille image ===
%     [X,Z] = meshgrid(X_m,obj.z);
%     Xc = mean(X_m);
%     Ireconstruct = zeros(size(X),'like',X);
% 
%     figure;
% 
%     % === Rétroprojection ===
...
% 
%         % Onde plane de structuration
%         h0 = exp(1i*2*pi*decimation(i)*df0x*S);
% 
%         % Interpolations
%         proj_sup = interp1(obj.z,Fsup(:,i),T(:),'linear',0);
%         proj_sup = reshape(proj_sup,size(X));
% 
%         proj_inf = interp1(obj.z,Finf(:,i),T(:),'linear',0);
%         proj_inf = reshape(proj_inf,size(X));
% 
%         % Rétroprojection
%         Ireconstruct = Ireconstruct + real(2 * h0 .* proj_sup) + real(proj_inf);
% 
%         % === Monitoring visuel ===
%         imagesc(X_m*1e3,obj.z*1e3,real(Ireconstruct))
%         colormap(parula)
%         colorbar
%         title(['angle = ',num2str(round(theta(i)*180/pi)), ...
%                '°, decimation = ',num2str(decimation(i))])
%         xlabel('x (mm)')
%         ylabel('z (mm)')
%         ylim(obj.Lz*1e3)
%         drawnow
...
function Ireconstruct = iRadon(obj, X_m , DelayLAWS , ActiveLIST , df0x , Lobject )
        ...
        % decimation : integer list of structuratuion, such that fx = decimation*df0x
        % fundamental structuration frequency : df0x SI
        % theta : list of angular values
        
        DelayLAWS_  = obj.SqueezeRepeat( DelayLAWS  ) ; % 4-phases reduction
        ActiveLIST_ = obj.SqueezeRepeat( ActiveLIST ) ; % 4-phases reduction
       
        [theta , ~ ,~ , M0 ] = EvalDelayLawOS_shared( X_m  , DelayLAWS_  , ActiveLIST_ , obj.c ) ;
    
        % compression datas into single structuration (4-phase measurement)
        [ F_ct_kx , theta , decimation ] = obj.AddSinCos( obj.R ) ;
        
        obj.F_R =  obj.fourierz( F_ct_kx ) ; % fourier transform along z before filtering
        
        I0 = decimation == 0; % find componant fx = 0 (pure plane wave) for center of fourier plane
        Fin         =  obj.F_R ;
        ...
        Hsup = ( FZ >= 0 ) ; %          
        % Lobject = 0.2e-3;
        FILTER = obj.GetFILTER(Lobject,size(Fin,2));
...
%  A = axes ; 
   for i= 1:length(theta)
       ...
       % filter properly       
       % Xc : absisse of center of the probe
       % M0 fixed point in the rotation of angle theta(i)
      ...
      % common interpolation:  
        ...
        projContrib_sup = interp1(obj.z,Fsup(:,i),T(:),'linear',0);
        projContrib_sup = reshape(projContrib_sup,length(obj.z),length(X_m));
       
        projContrib_inf = interp1(obj.z,Finf(:,i),T(:),'linear',0);
        projContrib_inf = reshape(projContrib_inf,length(obj.z),length(X_m));
       ...
       imagesc( X_m*1e3,obj.z*1e3,real(Ireconstruct))%real(Ireconstruct)
       colormap(parula)
       cb = colorbar ;
    ...
    set(findall(gcf,'-property','FontSize'),'FontSize',15) 
        ...
        function [Mout,ia,ib,theta,decim] = SqueezeRepeat(obj,Min)
           ...
           % ScanParam = [obj.decimation(:),obj.theta(:)] (number of scans performed)
                     ...
                     decimation = double(obj.decimation(:)); % Force le type double et vecteur colonne
                    theta = double(obj.theta(:));           % Force le type double et vecteur colonne
                
                    % 2. Création de ScanParam (matrice [N, 2] de type double)
                    ScanParam = [decimation, theta];
                
                    % 3. Utilisation de unique avec 'rows' pour comparer les lignes
        ...
        function [Iout,theta,decim] = AddSinCos(obj,Iin)
           ...
           theta = double(Angles(:,2))  ; % same size as ia 
           thetaUniq = unique(theta,'rows') ;
           
           decim = double(Angles(:,1))  ; % same size as ia 
