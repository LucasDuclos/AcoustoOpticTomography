classdef OP < TF_t
    ...
    methods
        ...
        function obj = OP(InputImage,theta,z,SamplingRate,c)
            %N: number of points for fourier transform
%              N = 2^10 ;
%              Fmax = 1/(t(2)-t(1));
%              obj@TF_t(N,Fmax);
            obj@TF_t( z ); % spatial unit for FFT
            
            if (size(InputImage) == [length(z),length(theta)])
            % checkin that dimension match the input image
            obj.SamplingRate = SamplingRate ;
            obj.z = z; % longitudinal index for reconstruction box
            obj.c = c ;
            obj.L = [min(z),max(z)] ;
            obj.R = InputImage ;
        ...
        function [] = Show_R(obj)
           Hf = figure(1); clf ;
           imagesc(obj.theta*180/pi,obj.z*1e3,obj.R)
           xlabel('theta (°)')
           ylabel('ct (mm)')
           title('Simulated Radon Measurement')
           cb = colorbar ;
           ylabel(cb,'tagged signal a.u')
        ...
        function [] = Show_F_R(obj,Fc)
           ...
           subplot(2,2,3)
           surfc(1e3./obj.f( I ) ,obj.theta*180/pi,abs(obj.F_R( I ,:))');
           shading interp
           view(0,90)
           ylabel('theta (°)')
           xlabel('\lambda (mm)')
...
N       = 2^12;
Fc      = 1/Lobject;  % Lobject in SI is the size of the object to detect. Using simple model (sinc function)
...
% obj.Show_R()
% obj = obj.PhaseCorrection(Fc);
% obj.Show_F_R(Fc); % Fc : cut-off frequency used for screening
