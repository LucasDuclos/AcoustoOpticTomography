...
try:
    ...
    expected = np.array([2*1 + 3*4, 2*2 + 3*5, 2*3 + 3*6])  # [14, 19, 24]
    ...
    expected = np.array([0.5, 2e6, 0.75, 4e6])  # y/max(q, threshold): 1/2=0.5, 2/1e-6=2e6, 3/4=0.75, 4/1e-6=4e6
...
try:
    ...
    # Note: We can't test the actual matrix creation without a manip object
...
try:
    ...
    # Test with a simple dense matrix
    np.random.seed(42)
    T, Z, X, N = 5, 10, 10, 5
    SMatrix = np.random.rand(T, Z, X, N).astype(np.float32)  # (T, Z, X, N)
    # y should have shape (T, N) to match the forward projection A @ theta
