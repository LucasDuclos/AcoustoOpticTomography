...
def PPGMLEM(
    ...
    for it in iterator:
        ...
        # PPGMLEM update: theta_new = theta + alpha * (A^T * (y / (A*theta + eps) - 1)) / (A^T * 1 + beta * diag(H_U))
        # Compute sensitivity (A^T * 1)
        sensitivity = backward_projection(SMatrix, array_module.ones_like(q_flat))
        
        # PPGMLEM update
        lambda_flat = lambda_flat + alpha * (c_flat - 1) / (sensitivity + beta * hess_U + gamma)
        ...
        if isCostFunction:
            cost_history.append(cost_function(SMatrix, lambda_flat, y_flat, optimizer=OptimizerType.PPGMLEM, array_module=array_module, beta=beta, delta=delta))
