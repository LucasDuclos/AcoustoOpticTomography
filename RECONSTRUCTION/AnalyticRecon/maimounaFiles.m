classdef OS < TF2D
    ...
    methods
        ...
        function obj = OS(InputImage,theta,decimation,df0x,t,SamplingRate,c,Lx)
            %N: number of points for fourier transform
%            N = 2^10 ;
%            Fmax_x = N*df0x;
%            Fmax_z = 1/(c*(t(2)-t(1))) ;
            %obj@TF2D(N,N,Fmax_x,Fmax_z);
            obj@TF2D( linspace(Lx(1),Lx(2),2^10),c*t);
            
            if (size(InputImage,1) == length(t) && size(InputImage,2) == length(theta))
            % checkin that dimension match the input image
            obj.SamplingRate = SamplingRate ;
            obj.ct = c*t; % longitudinal index for reconstruction box
            obj.c = c ;
            obj.Lx = Lx ;
            obj.Lz = c*[min(t),max(t)] ;
            obj.R = InputImage;
            obj.theta = theta;          
...
%  A = axes ; 
   for i= 1:length(theta)
      ...
      % common interpolation:  
       ...
       colormap(parula)
       cb = colorbar ;
       title(['angle(\B0): ',num2str(theta(i)*180/pi)])
       ylim(obj.Lz*1e3)
       xlabel('x (mm)')
       ylabel('z (mm)')
