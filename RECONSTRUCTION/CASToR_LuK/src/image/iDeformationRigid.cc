...
void iDeformationRigid::ShowHelp()
{
  ...
  cout << "It requires 3 translation (x,y,z) and 3 rotation parameters, provided by command-line option or in a text file" << endl;
  cout << endl;
  cout << "--------------------------------------------------------" << endl;
  ...
  cout << endl;
  cout << "Rotation convention: Any combinations of 'x', 'y', and 'z'" << endl;
  cout << "Define the axe on which the rotation will be performed using rotA, rotB and rotC angle in 1 (default): XYZ" << endl;
  cout << "Compute transformations: (1 or 0)" << endl;
  cout << "1 (default): Provided transformations parameters correspond to transformations i to i+1." << endl;
  cout << "             They will be recomputed to represent transformations from 0 (ref) to i  as required by CASToR" << endl;
  cout << "0          : Provided transformations already provided as 0 (ref) to i (forward) and i to 0 (backward) transformations." << endl;
}
...
int iDeformationRigid::ReadAndCheckConfigurationFile(const string& a_fileOptions)
{
  ...
  if(in_file)
  {
    ...
    if (ReadDataASCIIFile(a_fileOptions, "Transformation parameters", p_coeffs, 6, m_nbTransformations, KEYWORD_MANDATORY))
    {
    ...
    // Read 6 parameters for each transformations, on each line
    if (ReadDataASCIIFile(a_fileOptions, "Compute transformations", &m_cmpTransfoFlag, 1, KEYWORD_OPTIONAL) == KEYWORD_OPTIONAL_ERROR)
    {
    ...
    // Read 6 parameters for each transformations, on each line
    if (ReadDataASCIIFile(a_fileOptions, "Rotation convention", &m_rotConvention, 1, KEYWORD_OPTIONAL) == KEYWORD_OPTIONAL_ERROR)
    {
...
int iDeformationRigid::ReadAndCheckOptionsList(const string& a_listOptions)
{
  ...
  //
  if(m_verbose >=3) Cout("iDeformationRigid::ReadAndCheckOptionsList()-> Read command-line options parameters ..."<< endl);
...
int iDeformationRigid::ApplyDeformations(FLTNB* ap_inputImage, FLTNB* ap_outputImage, int a_direction, int a_defIdx)
{
  ...
  if(m_verbose >=3) Cout("iDeformationRigid::ApplyDeformations #" << a_defIdx+1 << " ("
...
int iDeformationRigid::TransformImage(int a_direction, int a_defIdx, HPFLTNB *ap_inputImage, HPFLTNB *ap_outputImage)
{
  ...
  if(m_verbose >=4)
  {
    Cout("iDeformationRigid::TransformImage using the following coefficients file : "<< endl); 
