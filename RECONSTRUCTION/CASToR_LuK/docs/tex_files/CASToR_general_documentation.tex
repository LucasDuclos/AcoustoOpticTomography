...
\begin{itemize}
  ...
  \item Optimization algorithm plug-in class system, including most well-known algorithms such as MLEM~\cite{MLEM}, One-Step-Late~\cite{OSL} and MLTR~\cite{MLTR} optimization, and also many others.
...
\begin{itemize}
  \item \textbf{number of rings in scanner}: for each layer, number of axial rows of elementary detection units (typically, crystal rings in PET).
...
where, \textit{a} corresponds to the slanthole unique coefficient defining the angle of the collimator, while \textit{sign(a)} defines the angle orientation. 
...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{PET datafile format (current based on crystal IDs)}
\label{ss_input_PET}
...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{PET datafile format (new one directly based on cartesian coordinates)}
\label{ss_input_PET_cart}

Here, PET data must not be linked to a particular PET scanner in CASToR.
The idea is that for each generic PET event, a set of cartesian coordinates defining the two end points of a line are supplied.
There is thus no need for any scanner description.
There also cannot be any compression.
The datafile can only be of histogram type.
...
\begin{itemize}
  \item \textbf{Data filename}: The absolute or relative path to the binary datafile
  \item \textbf{Number of events}: The number of CASToR events in the binary datafile
  \item \textbf{Data mode}: histogram
  \item \textbf{Data type}: PET
  \item \textbf{Start time (s)}: The relative start time of the acquisition in seconds (ignored for normalization mode)
  \item \textbf{Duration (s)}: The duration of the acquisition in seconds (ignored for normalization mode)
\end{itemize}
...
Table \ref{table_PET_data_bin_histogram_cart} describes the binary content of an event from a histogram datafile.
Note that little endian encoding only is supported. Whether an optional data field is present or not, the order of the fields provided in these tables
must be observed. The meaning of the type \textit{FLTNBDATA} is explained in section~\ref{s_install}.

Regarding time-of-flight information, TOF delta time measurement $\Delta t$ is defined as the difference in arrival time between point 1 and point 2, being positive when the emission (probably) occurs closer to crystal 2. In the TOF histogram binary file, TOF bins should be ordered so as to increase with $\Delta t$. Therefore, the first TOF bin contains counts (probably) emitted closest to crystal 1.

\begin{table} [h!]
  \small
  \caption{Mandatory/Optional fields of a PET histogram event}
  \label{table_PET_data_bin_histogram_cart}
  \begin{center}
  \begin{tabular}{|c|c|c|c|c|}
  \hline
    & \textbf{Symbol} & \textbf{Description} & \textbf{Type} & \textbf{Mandatory}\\
  ...
  5.2 & s & Un-normalized scatter intensity rate & FLTNBDATA & no \\
      & & of the corresponding histogram (TOF) bin (count/s) & & \\ \hline
  6.1 & x1 & X coordinate of point 1 & FLTNBDATA & yes \\ \hline
  6.2 & y1 & Y coordinate of point 1 & FLTNBDATA & yes \\ \hline
  6.3 & z1 & Z coordinate of point 1 & FLTNBDATA & yes \\ \hline
  7.1 & x2 & X coordinate of point 2 & FLTNBDATA & yes \\ \hline
  7.2 & y2 & Y coordinate of point 2 & FLTNBDATA & yes \\ \hline
  7.3 & z2 & Z coordinate of point 2 & FLTNBDATA & yes \\ \hline
  \end{tabular}
  \end{center}
\end{table}
