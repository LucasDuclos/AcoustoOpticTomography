...
//  cout << "  -help-dynamic        : Print out specific help about dynamic methodologies settings." << endl; // managed by main
  ...
  cout << "  -help-scan           : Print out the list of all scanners from the configuration directory." << endl; // managed by sScannerManager
  cout << endl;
...
void ShowHelpDynamic()
{
  ...
  cout << "                         Concatenate 's' or 'm' to specify if values are seconds or minutes (seconds is the default if not provided).  " << endl;
...
*/
  ...
  cout << endl;
  cout << "  -help-dmodel         : Print out specific help about dynamic model." << endl; // managed by oDynamicModelManager
  cout << endl;
...
int main(int argc, char** argv)
{
  ...
  // Ignore scanner
  bool ignore_scanner = false;
  // Data-type
  int data_type = -1;
  // Path to the system matrix directory
  string path_to_system_matrix_directory = "";
  ...
  for (int i=1; i<argc; i++)
  {
    ...
    else if (option=="-data-type")
    {
      ...
      string data_type_str = (string)argv[i+1];
      i++;
      if ( data_type_str=="PET" || data_type_str=="pet" || data_type_str=="0" )
	  {
	    data_type = TYPE_PET;
	  }
	  else if ( data_type_str=="SPECT" || data_type_str=="spect" || data_type_str=="1" )
	  {
	    data_type = TYPE_SPECT;
	  }
	  else if ( data_type_str=="CT" || data_type_str=="ct" || data_type_str=="2" )
	  {
	    data_type = TYPE_CT;
	  }
	  else if ( data_type_str=="AOT" || data_type_str=="aot" || data_type_str=="3" )
	  {
	    data_type = TYPE_AOT;
	  }
	  else
	  {
        Cerr("***** Unknown argument for option: " << option << endl);
        Exit(EXIT_FAILURE);
      }
	}
	// Path to the system matrix directory
	else if (option=="-system-matrix")
    {
      if (i>=argc-1)
      {
        Cerr("***** Argument missing for option: " << option << endl);
      ...
      path_to_system_matrix_directory= argv[i+1];
      i++;
    }
  ...
  // If system matrix is used, check if the data type is AOT
  if ( ignore_scanner && data_type != TYPE_AOT )
  {
    Cerr("***** castor-recon() -> system matrix is used with a data type different from AOT!" << endl);
    Exit(EXIT_FAILURE);
  }
  if (!(path_to_system_matrix_directory.empty()) && path_to_system_matrix_directory.back() != '/')
  {
    path_to_system_matrix_directory.append("/");
  }
  ...
  else
  {
		if (nb_voxX<=0 || nb_voxY<=0 || nb_voxZ<=0)
		{
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "voxels number transaxial", &nb_voxX, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "voxels number transaxial 2", &nb_voxY, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "voxels number axial", &nb_voxZ, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
		...
		if ( (fov_sizeX<=0 || fov_sizeY<=0 || fov_sizeZ<=0) && (vox_sizeX<=0 || vox_sizeY<=0 || vox_sizeZ<=0) )
		{
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "field of view transaxial", &fov_sizeX, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "field of view transaxial 2", &fov_sizeY, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
			if (ReadDataASCIIFile(path_to_system_matrix_directory+"field.hdr", "field of view axial", &fov_sizeZ, 1, KEYWORD_MANDATORY))
			{
				Cerr("***** castor-recon() -> A problem occurred while trying to find the  in the matrix header !" << endl);
				Exit(EXIT_FAILURE);
			}
  ...
  {
    Cerr("***** castor-recon() -> A problem occurred while checking parameters for the chrono manager !" << endl);
    Exit(EXIT_FAILURE);
  }
  if (p_ChronoManager->Initialize())
  {
    Cerr("***** castor-recon() -> A problem occurred while initializing the chrono manager !" << endl);
    Exit(EXIT_FAILURE);
  }
  ...
  {
	  if (data_type == TYPE_AOT)
	  {
		for (int i=0 ; i<nb_beds ; i++) p_DataFile[i] = new iDataFileAOT();
      }
      else
	  {
	    Cerr("***** castor-recon() -> Unexpected data type " << endl);
	    Exit(EXIT_FAILURE);
	  }
  }
  ...
  p_ProjectorManager->SetPathToSystemMatrixDirectory(path_to_system_matrix_directory);
  ...
  if (p_DataFile[0]->GetDataMode()==MODE_HISTOGRAM && p_OptimizerManager->GetNeedGlobalSensitivity()) sensitivity_from_histogram = true;
  ...
  if ( path_to_sensitivity_img.empty() && (p_DataFile[0]->GetDataMode()==MODE_LIST || sensitivity_from_histogram))
  {
    ...
    if ( p_DataFile[0]->GetDataMode()==MODE_LIST || p_OptimizerManager->GetNeedGlobalSensitivity())
