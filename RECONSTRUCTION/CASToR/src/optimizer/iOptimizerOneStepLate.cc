...
int iOptimizerOneStepLate::PreImageUpdateSpecificStep()
{
  ...
  {
    Cerr("***** iOptimizerOneStepLate::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        for (v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
        {
          ...
          {
            Cerr("***** iOptimizerOneStepLate::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
                                                         ...
                                                         INTNB a_voxel, int a_tbf, int a_rbf, int a_cbf )
{
  ...
  if (sensitivity_plus_penalty <= 0.)
  {
    ...
    if (m_displayWarningFlag)
    {
      ...
      Cerr("!!!!! This may be a sign that the penalty strength is to high, but it may only be due to voxels close to FOV extremities. !!!!!" << endl);
