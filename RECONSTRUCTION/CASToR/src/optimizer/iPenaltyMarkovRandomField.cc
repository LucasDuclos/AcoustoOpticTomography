...
void iPenaltyMarkovRandomField::ShowHelpSpecific()
{
  ...
  cout << "for parameters selection can be found in the configuration folder (config/penalty/MRF.conf). The configuration" << endl;
...
int iPenaltyMarkovRandomField::BuildNeighborhoodKernel()
{
  ...
  if (m_neighborhoodShape == MRF_NEIGHBORHOOD_SPHERE)
  {
    ...
    INTNB radius_vox_x = max( 1 , ((INTNB)(m_neighborhoodSphereRadius/vox_size_x)) );
    INTNB radius_vox_y = max( 1 , ((INTNB)(m_neighborhoodSphereRadius/vox_size_y)) );
    INTNB radius_vox_z = max( 1 , ((INTNB)(m_neighborhoodSphereRadius/vox_size_z)) );
...
int iPenaltyMarkovRandomField::LocalPreProcessingStep(int a_tbf, int a_rbf, int a_cbf, INTNB a_voxel, int a_th)
{
  ...
  {
    Cerr("***** iPenaltyMarkovRandomField::LocalPreProcessingStep() -> A problem occured while building specific neighborhood of voxel " << a_voxel << " !" << endl);
    return 1;
  }
  // Then compute the similarity factors
  if (ComputeSimilarityFactors(a_tbf,a_rbf,a_cbf,a_voxel,a_th))
  {
    Cerr("***** iPenaltyMarkovRandomField::LocalPreProcessingStep() -> A problem occured while computing the similirity factors of the neighborhood of voxel " << a_voxel << " !" << endl);
    return 1;
  }
  // Normal end
  return 0;
