...
int oOptimizerManager::ParseOptionsAndInitializeOptimizerAndPenalty()
{
  ...
  if (m_optionsPenalty!="")
  {
    ...
    else
    {
      ...
      // Build the default configuration file
      sOutputManager* p_output_manager = sOutputManager::GetInstance();
      file_options_penalty = p_output_manager->GetPathToConfigDir() + "/penalty/" + name_penalty + ".conf";
    }
    // ______________________________________________________________________________
...
int oOptimizerManager::PreImageUpdateStep()
{
  ...
  {
    Cerr("***** oOptimizerManager::PreImageUpdateStep() -> A problem occured while applying the pre-image-update step to the optimizer !" << endl);
    return 1;
  }
  // Normal end
  return 0;
