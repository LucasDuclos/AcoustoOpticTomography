...
void oProjectionLine::AddLine(int a_direction, int a_nbTOFBins, char* ap_voxelWeights)
{
	if (m_computationStrategy==IMAGE_COMPUTATION_STRATEGY)
	{
		// Work on a copy of the input pointer
		char* file_position = ap_voxelWeights;
		for (int tofbn=0; tofbn<a_nbTOFBins; tofbn++)
			for (int vn=0; vn<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); vn++)
			{
				m3p_voxelWeights[a_direction][tofbn][vn] += *reinterpret_cast<FLTNB*>(file_position);
				file_position += sizeof(FLTNB);
			}
	}
	else
	{
		Cout("***** oProjectionLine::AddLine() -> Only IMAGE COMPUTATION STRATEGY is implemented" << endl);
	}
}
