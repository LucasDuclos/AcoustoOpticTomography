     ...
     && ( alpha_XYZ[ 0 ] <= alpha_XYZ[ 2 ] ) )
    {
       ...
       && ( index_ijk[ 2 ] - 1 <= tof_index_high[2]  ) )
      {
        ...
        if (m_TOFWeightingFcnPrecomputedFlag)
        {
          ...
          INTNB temp = m_TOFWeightingFcnNbBins/2 + (INTNB)round( (alphaMid * length_LOR - lor_tof_center) * m_TOFPrecomputedSamplingFactor );
          // multiply the non TOF projection coefficient with the TOF weight
          if (temp>=0 && temp<m_TOFWeightingFcnNbBins) coeff *= mp_TOFWeightingFcn[temp];
          else coeff = 0.;
        }
        else
        {
          if (m_TOFBinProperProcessingFlag)
          {
            ...
            HPFLTNB prev_erf = erf((lor_tof_center - m_TOFBinSizeInMm/2. - alphaMid * length_LOR)/tof_sigma_sqrt2);
            HPFLTNB new_erf = erf((lor_tof_center + m_TOFBinSizeInMm/2. - alphaMid * length_LOR)/tof_sigma_sqrt2);
            // multiply the non TOF projection coefficient with the TOF weight
