...
void vProjector::ShowCommonHelp()
{
  ...
  cout << "------------------------------------------------------------------" << endl;
  cout << "Currently only TOF related options are available (if used, all of them must be specified): " << endl;
...
int vProjector::CheckParameters()
{
  ...
  {
    if (m_TOFResolutionInMm<=0. || m_TOFMeasurementRangeInMm<=0. || (!(m_TOFMethod==USE_TOFLIST && !m_TOFBinProperProcessingFlag) && m_TOFBinSizeInMm<=0.))
    {
...
int vProjector::Initialize()
{
  ...
  if (m_TOFMethod!=USE_NOTOF)
  {
    ...
    if (m_TOFWeightingFcnPrecomputedFlag)
    {
      ...
      m_TOFPrecomputedSamplingFactor = 100.;
      ...
      if (!m_TOFBinProperProcessingFlag)
      {
        ...
        m_TOFWeightingFcnNbBins = nb_samples_tof_gauss;
      }
      ...
      else
      {
        ...
        m_TOFWeightingFcnNbBins = nb_samples_conv;
