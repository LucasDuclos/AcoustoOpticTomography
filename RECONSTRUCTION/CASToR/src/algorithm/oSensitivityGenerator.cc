...
int oSensitivityGenerator::ComputeSensitivityFromScanner(int a_bed)
{
  ...
  {
    // Verbose
    if (m_verbose>=3 && mp_ImageDimensionsAndQuantification->GetNbTimeFrames()>1)
    ...
    {
      // Verbose
      if (m_verbose>=3 && mp_ImageDimensionsAndQuantification->GetNb1stMotImgsForLMS(fr)>1)
        cout << "oSensitivityGenerator::ComputeSensitivityFromScanner() -> Processing 1st motion image for sensitivity " << rg+1 << endl;
      // Loop on cardiac gates
      for (int cg=0 ; cg<mp_ImageDimensionsAndQuantification->GetNb2ndMotImgsForLMS() ; cg++)
      {
        // Verbose
        if (m_verbose>=3 && mp_ImageDimensionsAndQuantification->GetNb2ndMotImgsForLMS()>1)
        ...
        if(    ( fr==0 && rg==0 && cg==0 ) // First sensitivity image
            || (   m_mumapAttenuationFlag // Attenuation image provided, first frame, and gating/physiological motion correction enabled
               &&  fr==0 
               &&  (  mp_ImageDimensionsAndQuantification->GetDynRecoType() == DYN_RECO_GATING
                   || mp_ImageDimensionsAndQuantification->GetDynRecoType() == DYN_RECO_MCGATING) ) 
            || (   m_mumapAttenuationFlag // Attenuation image provided and involuntary motion correction enabled
               &&  mp_ImageDimensionsAndQuantification->GetDynRecoType() == DYN_RECO_IPMC )
          )
        {
          if (m_verbose>=3 && mp_ImageDimensionsAndQuantification->GetMPIRank()==0)
          ...
          {
            #ifdef CASTOR_MPI  
            if(idx_elt1==main_loop_start_index && m_verbose>=2) 
             Cout("oSensitivityGenerator::ComputeSensitivityFromScanner() -> MPI ID " << mp_ImageDimensionsAndQuantification->GetMPIRank() 
            ...
            for (int64_t idx_elt2=inner_loop_start_index ; idx_elt2<inner_loop_stop_index ; idx_elt2++)
            {
              ...
              if (th==0  && mp_ImageDimensionsAndQuantification->GetMPIRank()==0)
              {
                if (m_verbose>=2 && printing_index%10000==0)
                {
                  ...
                  if( m_mumapAttenuationFlag &&  (  mp_ImageDimensionsAndQuantification->GetDynRecoType() == DYN_RECO_GATING
                                                 || mp_ImageDimensionsAndQuantification->GetDynRecoType() == DYN_RECO_MCGATING) ) 
                    progression_index_current = p_scannerManager->PROJ_GetCurrentProgression(idx_elt1, idx_elt2, progression_elts_array, 
                                                                                             mp_ImageDimensionsAndQuantification->GetNb1stMotImgsForLMS(fr), 
                                                                                             mp_ImageDimensionsAndQuantification->GetNb2ndMotImgsForLMS(), 
                                                                                             fr, rg, cg);
                  ...
                  {
                    // TODO recover the total number of motion subset (!= of the total number of triggers due to framing)
                    progression_index_current = p_scannerManager->PROJ_GetCurrentProgression(idx_elt1, idx_elt2, progression_elts_array, 
                                                                                             mp_ImageDimensionsAndQuantification->GetNb1stMotImgsForLMS(fr), 
                                                                                             1, fr, rg, cg);
                  }
                  else
                    progression_index_current = p_scannerManager->PROJ_GetCurrentProgression(idx_elt1, idx_elt2, progression_elts_array, 1, 1, 0, rg, cg);
        ...
        else
        {
          ...
          // Framing, and if IPM is not enabled : duplicate all gates for each frame
          if ( fr>0 && mp_ImageDimensionsAndQuantification->GetDynRecoType() != DYN_RECO_IPMC) 
          {
