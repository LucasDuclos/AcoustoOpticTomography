...
{
  cout << "This optimizer is the BSREM (for Block Sequential Regularized Expectation Maximization) algorithm, in developpement." << endl;
  cout << "It follows the definition of BSREM II in Ahn and Fessler 2003" << endl;
  ...
  cout << "sensitivity loss at the edges of the field-of-view. As it is simply based on the gradient, penalty terms must" << endl;
  cout << "have a derivative order of at least one. Without penalty, it reduces to OSEM but where the sensitivity is not" << endl;
  cout << "dependent on the current subset. This is a requirement of the algorithm, explaining why it starts by computing" << endl;
  cout << "the global sensitivity before going through iterations. The algorithm is restricted to histograms." << endl;
...
int iOptimizerBSREM::PreImageUpdateSpecificStep()
{
  ...
  {
    Cerr("***** iOptimizerBSREM::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        for (v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
        {
          ...
          {
            Cerr("***** iOptimizerBSREM::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
        ...
        if (problem)
        {
          Cerr("***** iOptimizerBSREM::PreImageUpdateSpecificStep() -> A problem occured inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
                                                   ...
                                                   FLTNB a_sensitivity, FLTNB* ap_correctionValues, INTNB a_voxel, int tbf, int rbf, int cbf )
{
  ...
  HPFLTNB scaled_penalty = ((HPFLTNB)(m4p_firstDerivativePenaltyImage[tbf][rbf][cbf][a_voxel])) / ((HPFLTNB)(mp_nbSubsets[m_currentIteration]));
