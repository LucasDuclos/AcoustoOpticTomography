...
int vOptimizer::CheckParameters()
{
  ...
  // Check data type
  if (m_dataType==TYPE_UNKNOWN || (m_dataType!=TYPE_PET && m_dataType!=TYPE_SPECT && m_dataType!=TYPE_CT && m_dataType!=TYPE_AOT))
  {
...
int vOptimizer::PreImageUpdateStep()
{
  ...
  {
    Cerr("***** vOptimizer::PreImageUpdateStep() -> A problem occured while applying the specific post-data-update step of the optimizer module !" << endl);
    return 1;
  }
        ...
        for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
        {
          for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
          {
            ...
            for (v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
            {
              ...
              {
                Cerr("***** vOptimizer::PreImageUpdateStep() -> A problem occured while precomputing the local step of the penalty for voxel " << v << " !" << endl);
                problem = true;
              }
            ...
            if (problem)
            {
              Cerr("***** vOptimizer::PreImageUpdateStep() -> A problem occured in the multi-threaded loop for the penalty, stop now !" << endl);
              return 1;
            }
          }
        }
...
int vOptimizer::UpdateVisitedVoxels()
{
  ...
  // As this matter of visited voxels does not depend on dynamic aspects, we loop on the sensitivity only for the first
  // frame/gates and look if voxels were 'visited' by LORs (i.e. the sensitivity is not null).
  int thread0 = 0;
  int frame0 = 0;
  int resp_gate0 = 0;
  int card_gate0 = 0;
  for (int v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
    ...
    if (mp_ImageSpace->m5p_sensitivity[thread0][frame0][resp_gate0][card_gate0][v]!=0.) mp_ImageSpace->mp_visitedVoxelsImage[v] += 1.;
