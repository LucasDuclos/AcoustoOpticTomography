...
void iOptimizerPenalizedPreconditionedGradientML::ShowHelpSpecific()
{
  ...
  cout << "Thus, negative values cannot occur and voxels cannot be trapped into 0 values, providing the first estimate is strictly positive." << endl;
...
int iOptimizerPenalizedPreconditionedGradientML::PreImageUpdateSpecificStep()
{
  ...
  {
    Cerr("***** iOptimizerPenalizedPreconditionedGradientML::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        { 
          ...
          {
            Cerr("***** iOptimizerPenalizedPreconditionedGradientML::PreImageUpdateSpecificStep() -> A problem occured while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
        ...
        if (problem)
        {
          Cerr("***** iOptimizerPenalizedPreconditionedGradientML::PreImageUpdateSpecificStep() -> A problem occured inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
