...
void iPenaltyMedianRootPrior::ShowHelpSpecific()
{
  ...
  cout << "template configuration file for parameters selection can be found in the configuration folder (config/penalty/MRP.conf)." << endl;
...
int iPenaltyMedianRootPrior::LocalPreProcessingStep(int a_tbf, int a_rbf, int a_cbf, INTNB a_voxel, int a_th)
{
  ...
  {
    Cerr("***** iPenaltyMedianRootPrior::LocalPreProcessingStep() -> A problem occured while building specific neighborhood of voxel " << a_voxel << " !" << endl);
    return 1;
  }
  // Compute the median value in this neighborhood
  if (ComputeMedianInNeighborhood(a_tbf,a_rbf,a_cbf,a_voxel,a_th))
  {
    Cerr("***** iPenaltyMedianRootPrior::LocalPreProcessingStep() -> A problem occured while computing the median in neighborhood of voxel " << a_voxel << " !" << endl);
    return 1;
  }
  // Normal end
  return 0;
