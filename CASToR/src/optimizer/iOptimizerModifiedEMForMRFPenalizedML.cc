...
iOptimizerModifiedEMForMRFPenalizedML::iOptimizerModifiedEMForMRFPenalizedML() : vOptimizer()
{
  ...
  mp_sumOfWeightsImage = NULL;
//  m_startMAPFlag = false;
}
...
void iOptimizerModifiedEMForMRFPenalizedML::ShowHelpSpecific()
{
  ...
  cout << "Subsets can be used as for OSEM, without proof of convergence however." << endl;
...
int iOptimizerModifiedEMForMRFPenalizedML::InitializeSpecific()
{
  ...
  // -------------------------------------------------------------------  
  // Allocate and create the smoothed image
  ...
  { 
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        for (INTNB v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++) m4p_smoothedImage[tbf][rbf][cbf][v] = 0.;
      }
    }
  }

  // -------------------------------------------------------------------
  // The image containing the sum of neighbors weights contributing to the smoothed image
  mp_sumOfWeightsImage = mp_ImageSpace->AllocateMiscellaneousImage();
  for (INTNB v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++) mp_sumOfWeightsImage[v] = 0.;
  ...
  for (v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
  {
    ...
    #endif
    // Build neighbors list (do not check the return value as this function cannot fail, and we are in a threaded loop)
    p_mrf_penalty->BuildSpecificNeighborhood(v,th);
    ...
    for (INTNB n = 0; n<p_mrf_penalty->GetNeighborhoodMaxNbVoxels(); n++)
    {
      ...
      {
        // Get the proximity factor
        mp_sumOfWeightsImage[v] += p_mrf_penalty->GetProximityKernel()[n];
      }
    }
  }
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        for (v=0; v<mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(); v++)
        {
          ...
          for (INTNB n = 0; n<p_mrf_penalty->GetNeighborhoodMaxNbVoxels(); n++)
          {
            ...
            {
              // Get the proximity factor
              FLTNB prox = p_mrf_penalty->GetProximityKernel()[n];
              // Add the contribution of this neighbor to the sum
              weighted_sum += prox 
                           * ( mp_ImageSpace-> m4p_image[tbf][rbf][cbf][v] 
                             + mp_ImageSpace-> m4p_image[tbf][rbf][cbf][neighbor] );
            }
          }
          // Affect penalty image
          m4p_smoothedImage[tbf][rbf][cbf][v] = weighted_sum;
        }
        // Check for problems
        if (problem)
        {
          Cerr("***** iOptimizerOneStepLate::PreImageUpdateSpecificStep() -> A problem occured inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
  ...
  *ap_newImageValue = mlem_update;
  ...
  if (mp_Penalty!=NULL)
  {
    ...
    if (beta>0.)
    {
      ...
      *ap_newImageValue = (term + sqrt( term*term + 8.*((HPFLTNB)(mp_sumOfWeightsImage[a_voxel]))*beta*((HPFLTNB)(a_sensitivity))*mlem_update ))
                        / (4.*((HPFLTNB)(mp_sumOfWeightsImage[a_voxel]))*beta);
    }
    ...
    *ap_newImageValue = a_currentImageValue * image_update_factor;
    ...
    if (fpclassify(*ap_newImageValue)==FP_SUBNORMAL) *ap_newImageValue = 0.;
  }
