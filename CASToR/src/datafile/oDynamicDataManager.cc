...
int oDynamicDataManager::InitDynamicDataPatientMotion(const string& a_pathToFile)
{
  ...
  // Loop on all motion triggers (MT)
  for(int t=0 ; t<m_nbPMotionTriggers ; t++)
  {
    // Check if the current MT timestamp occurs after the current frame (fr index) timestop. 
    // In this case : Check the next frame, init the first and last MT index for the next frame as the current MT and MT+1
    // Otherwise : Out of while loop, increment the last index of this frame, and go to the next MT

    while( mp_listPMotionTriggers[t] > mp_ID->GetFrameTimeStopInMs(0,fr) )
    {
      // Increment frame index and define the trigger indexes as the first and last indexes of the frame 
      fr++;
      mp_framePMotionFirstIndex[fr] = t  ;
      mp_framePMotionLastIndex [fr] = t+1;
      //mp_frameNbPMotionTriggers[fr]=t+1;
      
      // Throw error if there is a motion trigger after the last frame TimeStop)
      if(fr >= m_nbTimeFrames)
      {
        Cerr("***** oDynamicDataManager::InitDynamicDataPatientMotion() -> Error, the "<<t+1<<"th motion trigger timestamp: "<< mp_listPMotionTriggers[t] 
           <<" occurs after the end of the last frame (frame "<<fr<<", timestop :"<< mp_ID->GetFrameTimeStopInMs(0,fr-1) << " !" << endl);
        return 1;
      }
    }
    
    // Increment the last MT index for the current frame index
    mp_framePMotionLastIndex[fr]++;
  }
   
  // Set the indexes of any remaining frames
  if(fr<m_nbTimeFrames-1)
    for(int f=fr ; f<m_nbTimeFrames ; f++)
    {
      mp_framePMotionFirstIndex[fr] = m_nbPMotionTriggers-1;
      mp_framePMotionLastIndex[fr] = m_nbPMotionTriggers;
    } 
  
  // Recover number of MT for each frame
  for(int fr=0 ; fr<m_nbTimeFrames ; fr++)
    mp_frameNbPMotionTriggers[fr] = mp_framePMotionLastIndex[fr]-mp_framePMotionFirstIndex[fr];
    

  // Compute the weight of each pm subsets within each frame (ratio subset duration/total frame duration)
  // This will be used only for list-mode sensitivity image generation
  int t=0;
  
  for(int fr=0 ; fr<m_nbTimeFrames ; fr++)
    m2p_listPMotionWeightInFrame[fr] = (HPFLTNB*)malloc(mp_frameNbPMotionTriggers[fr]*sizeof(HPFLTNB));
  

  for(int fr=0 ; fr<m_nbTimeFrames ; fr++)
  {
    uint32_t motion_subset_start_time = mp_ID->GetFrameTimeStartInMs(0,fr);
    uint32_t motion_subset_stop_time = 0;
    
    if (t<m_nbPMotionTriggers) // Not the last motion trigger. Then check if the next stop is the end of the frame or if another transformation occurs before.
      motion_subset_stop_time = (mp_listPMotionTriggers[t] < mp_ID->GetFrameTimeStopInMs(0,fr) )?
                                 mp_listPMotionTriggers[t] : 
                                 mp_ID->GetFrameTimeStopInMs(0,fr);
    else // No more motion trigger. Then the next stop is the end of the frame
      motion_subset_stop_time = mp_ID->GetFrameTimeStopInMs(0,fr) ;
                                      
                                         
    for( int ipm=0 ; ipm<mp_frameNbPMotionTriggers[fr] ; ipm++ )
    {
      if (t<m_nbPMotionTriggers) // Not the last motion trigger. Then check if the next stop is the end of the frame or if another transformation occurs before.
        motion_subset_stop_time = (mp_listPMotionTriggers[t] < mp_ID->GetFrameTimeStopInMs(0,fr) )?
                                   mp_listPMotionTriggers[t] : 
      ...
      m2p_listPMotionWeightInFrame[fr][ipm] = FLTNB(motion_subset_stop_time - motion_subset_start_time ) / mp_ID->GetFrameDurationInMs(0,fr) ;
      
      motion_subset_start_time = mp_listPMotionTriggers[t];
