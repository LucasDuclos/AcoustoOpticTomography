...
int vDataFile::ReadInfoInHeader(bool a_affectQuantificationFlag)
{
  ...
  else if ( data_type=="AOT" || data_type=="aot" || data_type=="3" )
  {
    m_dataType = TYPE_AOT;
    m_dataSpec = SPEC_EMISSION;
  }
  ...
  // Get scanner name (optional for AOT)
  ...
  {
	  if (ReadDataASCIIFile(m_headerFileName, "Scanner name", &m_scannerName, 1, KEYWORD_OPTIONAL) == 1 ) 
	  {
		Cerr("***** vDataFile::ReadInfoInHeader() -> Error while trying to read scanner name in the header data file !" << endl);
		return 1;
	  }
...
int vDataFile::CheckParameters()
{
  ...
  if (m_dataType!=TYPE_PET && m_dataType!=TYPE_SPECT && m_dataType!=TYPE_CT && m_dataType!=TYPE_AOT)
  {
...
string vDataFile::GetDataTypeToString()
{
  ...
  else if (m_dataType==TYPE_AOT) data_type = "AOT";
