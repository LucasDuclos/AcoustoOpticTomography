...
{
  // Check scanner
  if (mp_Scanner==NULL && m_optionsForward!=SYSTEM_MATRIX_KEYWORD)
  {
...
int oProjectorManager::Initialize()
{
  ...
  else if (mp_DataFile->GetDataType()==TYPE_AOT)
  {
	m_TOFMethod = USE_TOFHISTO;
	//iDataFileAOT* p_aot_datafile = (dynamic_cast<iDataFileAOT*>(mp_DataFile));
    m_nbTOFBins = (dynamic_cast<iDataFileAOT*>(mp_DataFile))->GetNbTOFBins();
  }
  // Not PET data
  else
  {
...
int oProjectorManager::ParseOptionsAndInitializeProjectors()
{
  ...
  if (m_forwardProjectorName==SYSTEM_MATRIX_KEYWORD)
  {
    ...
    mp_SystemMatrixForward = new oSystemMatrix();

    // Load system matrix
	if (m_verbose>=1) Cout("oProjectorManager::ParseOptionsAndInitializeProjectors() -> Load the matrix system" << endl);
	if (mp_SystemMatrixForward->LoadData(mp_DataFile, mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(), m_pathToSystemMatrixDirectory))
	{
	  Cerr("***** oProjectorManager::ParseOptionsAndInitializeProjectors() -> A problem occurred while loading the matrix system" << endl);
      return 1;
	}
  }
  ...
  else
  {
    ...
    if (m_backwardProjectorName==SYSTEM_MATRIX_KEYWORD)
    {
      ...
      mp_SystemMatrixBackward = new oSystemMatrix();

      // Load system matrix
	  if (m_verbose>=1) Cout("oProjectorManager::ParseOptionsAndInitializeProjectors() -> Load the matrix system for backward projection" << endl);
	  if (mp_SystemMatrixBackward->LoadData(mp_DataFile, mp_ImageDimensionsAndQuantification->GetNbVoxXYZ(), m_pathToSystemMatrixDirectory))
	  {
	    Cerr("***** oProjectorManager::ParseOptionsAndInitializeProjectors() -> A problem occurred while loading the matrix system for backward projection" << endl);
        return 1;
	  }
    }
    // ______________________________________________________________________________
...
oProjectionLine* oProjectorManager::ComputeProjectionLine(vEvent* ap_Event, int a_th) 
{
  ...
  if (m_useProjectorForward)
  {
	  ...
	  index1 = ap_Event->GetEventID1();
	  index2 = ap_Event->GetEventID2();
	  nb_indices = ap_Event->GetNbLines();
  ...
  {
	  if (dynamic_cast<iEventHistoAOT*>(ap_Event)->GetEventIdentifier(eventIdentifier))
	  {
		Cerr("***** oProjectorManager::ComputeProjectionLine() -> A problem occurred while getting the name of the event !" << endl);
		return NULL;
	  }
  }
  ...
  if (m_useProjectorForward)
    ...
    return_value = mp_SystemMatrixForward->Project( FORWARD, m2p_ProjectionLines[a_th], eventIdentifier );
  if (return_value)
  {
  ...
  if (!m_useMatchedProjectors)
  {
    ...
    if (m_useProjectorBackward)
      ...
      return_value = mp_SystemMatrixBackward->Project( BACKWARD, m2p_ProjectionLines[a_th], eventIdentifier );
    if (return_value)
    {
