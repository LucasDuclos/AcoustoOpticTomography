...
int iProjectorDistanceDriven::ProjectTOFListmode(int a_direction, oProjectionLine* ap_ProjectionLine)
{
  ...
  if (m_TOFWeightingFcnPrecomputedFlag) tof_half_span = ((HPFLTNB)m_TOFWeightingFcnNbBins)/(2.*m_TOFPrecomputedSamplingFactor);
  ...
  if( ( angle >= 0.0 && angle <= 45.0 ) || ( angle >= 135.0 && angle <= 180.0 ) )
  {
    ...
    for( int16_t i = i_min; i < i_max; ++i )
    {
      ...
      if (m_TOFWeightingFcnPrecomputedFlag)
      {
        ...
        INTNB temp = m_TOFWeightingFcnNbBins/2 + (INTNB)round( (step * factor_for_tof - lor_tof_center) * m_TOFPrecomputedSamplingFactor );
        if (temp>=0 && temp<m_TOFWeightingFcnNbBins) tof_weight = mp_TOFWeightingFcn[temp];
      }
      else
      {
        if (m_TOFBinProperProcessingFlag)
        {
          ...
          HPFLTNB prev_erf = erf((lor_tof_center - m_TOFBinSizeInMm/2. - step * factor_for_tof)/tof_sigma_sqrt2);
          HPFLTNB new_erf = erf((lor_tof_center + m_TOFBinSizeInMm/2. - step * factor_for_tof)/tof_sigma_sqrt2);
    ...
    for( int16_t i = i_min; i < i_max; ++i )
    {
      ...
      for (INTNB tof_bin=tof_bin_first_for_voxel; tof_bin<=tof_bin_last_for_voxel; tof_bin++)
      {
        if (m_TOFWeightingFcnPrecomputedFlag)
        {
          ...
          // nearest to the projection of the voxel center onto the LOR
          INTNB temp = m_TOFWeightingFcnNbBins/2 + (INTNB)round( (step * factor_for_tof - lor_tof_center) * m_TOFPrecomputedSamplingFactor );
          if (temp>=0 && temp<m_TOFWeightingFcnNbBins) tof_weights_temp[tof_bin] = mp_TOFWeightingFcn[temp];
