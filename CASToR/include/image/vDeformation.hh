...
class vDeformation
{
  ...
  public:
    ...
    */
    ...
    /*!
      \fn      virtual int vDeformation::PerformDeformationIpat()
      \param   ap_Image : required to access oImageSpace image matrices
      \param   a_defIdx : index of the deformation
      \param   a_fr : frame index
      \param   a_rimg : respiratory image index
      \param   a_cimg : cardiac image index
      \brief   Just Apply forward deformation of the forward image with defIdx \n
               Used only for time-stamp based deformation (involuntary patient motion)
      \return  0 if success, positive value otherwise
    */
    virtual int PerformDeformationIpat(oImageSpace* ap_Image, int a_defIdx, int a_fr, int a_rimg, int a_cimg);
